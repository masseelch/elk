...
This package provides an extension to the [awesome entgo.io](https://github.com/ent/ent).

`elk`s goal is to generate a fully compliant, extendable [OpenAPI](https://spec.openapis.org/oas/v3.0.3.html)
specification file to enable the user to make use of the awesome [Swagger Tooling](https://swagger.io/tools/) to
generate RESTful servers and clients. Additionally, `elk` can generate a ready-to-use and extendable server
implementation behaving as described by the OpenAPI specification. The code generated by `elk` uses the Ent ORM while
maintaining complete type-safety and leaving reflection out of sight.
...
the [docs](https://pkg.go.dev/github.com/masseelch/elk) [integration test setup](https://github.com/masseelch/elk/tree/master/internal/integration/pets/ent)
.
