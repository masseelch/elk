...
func (h *MediaHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	es, err := q.Limit(itemsPerPage).Offset((page - 1) * itemsPerPage).All(r.Context())
	if err != nil {
		l.Error("error fetching media from db", zap.Error(err))
		InternalServerError(w, nil)
		return
	}
	l.Info("media rendered", zap.Int("amount", len(es)))
	easyjson.MarshalToHTTPResponseWriter(NewMedia1941033838Views(es), w)
}
