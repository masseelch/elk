...
// Delete removes a ent.Media from the database.
func (h MediaHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	err = h.client.Media.DeleteOneID(id).Exec(r.Context())
	if err != nil {
		switch {
		case ent.IsNotFound(err):
			msg := stripEntError(err)
			...
			NotFound(w, msg)
		default:
			l.Error("could-not-delete-media", zap.Error(err), zap.Int("id", id))
			InternalServerError(w, nil)
		}
		return
	}
	l.Info("media deleted", zap.Int("id", id))
	w.WriteHeader(http.StatusNoContent)
}
