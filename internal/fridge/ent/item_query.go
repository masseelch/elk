...
func (iq *ItemQuery) QueryCompartment() *CompartmentQuery {
	query := &CompartmentQuery{config: iq.config}
...
func (iq *ItemQuery) Clone() *ItemQuery {
	...
	return &ItemQuery{
		...
		withCompartment: iq.withCompartment.Clone(),
...
func (iq *ItemQuery) sqlAll(ctx context.Context) ([]*Item, error) {
	var (
		...
		_spec       = iq.querySpec()
		loadedTypes = [1]bool{
			iq.withCompartment != nil,
		}
	)
	if iq.withCompartment != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := iq.withCompartment; query != nil {
