...
var (
	...
	// FuncMap contains the extra template functions used by elk.
	FuncMap = template.FuncMap{
		...
		"hasEdgeOperations": hasEdgeOperations,
		"pluralize":         rules.Pluralize,
		"singularize":       rules.Singularize,
		// "view":            newView,
		// "views":           newViews,
	...
	HTTPTemplates = gen.MustParse(gen.NewTemplate("elk").Funcs(FuncMap).ParseFS(templateDir, "template/*.tmpl"))
)

var rules = inflect.NewDefaultRuleset()

type Operations []entoas.Operation

func (ops Operations) Has(o entoas.Operation) bool {
	for _, op := range ops {
		if op == o {
			return true
		}
...
func nodeOperations(n *gen.Type) (Operations, error) { return entoas.NodeOperations(n) }

func edgeOperations(e *gen.Edge) (Operations, error) { return entoas.EdgeOperations(e) }

func hasEdgeOperations(g *gen.Graph, n *gen.Type) (bool, error) {
	for _, t := range g.Nodes {
		if t.Name != n.Name {
			continue
		}
		for _, e := range n.Edges {
			ops, err := entoas.EdgeOperations(e)
			if err != nil {
				return false, err
			}
			if len(ops) > 0 {
